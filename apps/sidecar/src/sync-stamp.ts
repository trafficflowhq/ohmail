 ...
 * naturally once-only and safe against two passes finishing close together — and its `RETURNING`
 * is therefore the authority on whether the import finished HERE. The first statement's
 * `RETURNING` carries the stamp as it stood, which costs no extra read: the pass write does not
 * touch that column.
 */
