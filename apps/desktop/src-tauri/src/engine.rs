...
pub fn attach<R: tauri::Runtime>(builder: tauri::Builder<R>) -> tauri::Builder<R> {
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // Settings → Updates. A read of the update flow's state, and a press that means
            // exactly what picking the menu item means. `updater.rs` carries the reasoning —
            // including why a shell whose window reaches no network can safely own both.
            crate::updater::update_state,
            crate::updater::update_press,
            // …and the scheduled check, which is the launch check's own path rather than
            // the press's: silent unless it finds something. A press is a person asking
            // and is answered out loud, which is wrong once a day forever.
            crate::updater::update_poll
        ])
}
