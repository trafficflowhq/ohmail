...
import { messageStates, recordChange, type Tx } from "@trafficflow/db";
 ...
 * server-side the moment the user reads or replies to the row — see `MessageService.markSeen` —
 * and deliberately NOT on merely opening it, so a glance does not spend the resurface.
 *
