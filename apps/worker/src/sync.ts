...
import { junkRescuePass } from "./junk-rescue.js";
import {
  RECONCILE_BACKOFF_MINUTES, classifyMoveRefusal, isTransportFailure, nextReconcileAttemptAfter,
} from "./reconcile-refusal.js";
/* Re-exported where it has always lived: the ladder's one consumer outside this file is a test
...
export type CyclePass = "folder_ops" | "junk_rescue" | "junk_sweep" | "filing" | "flags";
...
async function syncCycleWithin(
  ...
  // ── AND THE QUEUED "NOT JUNK" PRESSES, IN THE SAME SLOT (FOLDERS-SPEC.md §16.2) ────────────
  //
  // Same place as the folder verbs and for the same reason: a USER-COMMANDED act the organizer
  // executes BEFORE the cursor is built, so this cycle's `changesSince` already observes the
  // rescued message arriving in INBOX with its bytes. Driven from the REPO and not through an
  // optional `SyncDeps` port: `junkSweep` above is wired in one composition alone, so on the
  // desktop and standalone doors that command is never drained and a queued row renders "Will be
  // moved to your inbox" for ever. `!readerMode` on the block above's argument, in full — the
  // press was recorded by an install that was the organizer then, so a demotion since must not
  // turn it into a move on a mailbox somebody else now organizes.
  let junkRescueOwesMore = false;
  if (!readerMode) {
    try {
      // ONE SLICE, ONE PAGE — the pass asks the lease before every move it issues; what the cursor
      // records is that a refusal inside it belongs to this cycle's junk_rescue page.
      openPage(at, "junk_rescue");
      const out = await junkRescuePass({
        repo, adapter, accountId, mailboxId,
        ...(log !== undefined ? { log } : {}),
        write: (fn) => fencedLiveGroup(deps, fn),
        writeAuthority: writeAuthorityOf(deps),
      });
      junkRescueOwesMore = out.owesMore;
      if (out.moved + out.voided + out.deferred + out.refused > 0) {
        log?.info("junk_rescue_pass_ran", {
          mailboxId, accountId,
          moved: out.moved, voided: out.voided, deferred: out.deferred, refused: out.refused,
        });
      }
    } catch (err) {
      rethrowRefusal(err);
      log?.warn("junk_rescue_pass_failed", { mailboxId, accountId, err });
    }
  }
  ...
  return {
    ...
    owesFiling: owesMore || folderOpsOweMore || junkRescueOwesMore || sweepOwesMore,
  };
}
