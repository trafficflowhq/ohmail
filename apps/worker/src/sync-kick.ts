 ...
 * the send's finalize, the junk rescue, the junk sweep press, the folder-op service and the mailbox
 * service's resync. Ordinary FILING does not — those writers commit desired state and stop, so filed mail waits ~105 s for its turn, a real gap left open deliberately. NOT a leader-locked backstop — it runs
