 ...
 * so no single `PATCH /messages` can exceed the route's cap. Each `engine.mutate` mints its OWN Idempotency-Key
 * (`OhmailEngine.mutate`), so a chunk whose response is lost is replayed under the same key and never double-applies.
 */

/**
 * The read state reaches `\Seen` on the user's IMAP via the worker's flag reconciler; the API never opens an IMAP
 * connection itself. `unread: false` is the read direction. Ids are flipped optimistically by the folder-agnostic
 * `mark_seen` mutation (`client-engine/mutations.ts`), which flips exactly the ids it is given. Returns the number of
 * chunks dispatched — the guard asserts `ceil(ids / 200)`.
 */
