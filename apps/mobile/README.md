...
**Status: pairs with your own server, reads and triages real mail; the demo
world is the default.** Out of the box the app renders Mila's demo from
`@ohmail/fixtures`, and that mode makes no network requests. Pair it with a
desktop install or a self-hosted server (**More → Connect to a server** — a QR
scan, a pasted pairing link, or an address typed by hand; over the LAN or a
tailnet) and it becomes a real client: the same `@ohmail/client-engine` every
other client runs syncs your mail into an on-device, per-account sqlite mirror,
and the Ohbox, Reads, Receipts and Screener screens read and triage it —
mark-read, screening decisions, releases and piles all round-trip to the
server and survive an app kill.

Honest edges, stated here and on the screens themselves:

- **Search and Settings still render the demo world**, even while a session is
  live. Both screens say so in place. Live search and per-server settings
  arrive with later updates.
- **Compose/reply, tags and attachment-open are not live yet** — the demo
  keeps them; a live screen offers no control it cannot perform.
...
The app says what it is on its own first screen, not only here.
...
| **`@ohmail/fixtures` consumed live** from the workspace | One demo corpus, one privacy review. Vendoring a copy would fork the demo world across two files. See the Metro note below. |
...
| **No state library** | `src/state/model.ts` is a pure state machine for the demo; `store.tsx` is a 60-line React binding over it. Live state is the engine's own mirror behind `src/state/live.ts`. |
| **Demo mode keeps no state and opens no socket** | Quitting resets the demo world — the honest behaviour for a fixture preview. A connected session is different on purpose: its mirror persists in sqlite, one database per (server, account), stamped with its owner and wiped on a mismatch. |
...
Metro the TypeScript `extensionAlias` rule so `@ohmail/fixtures`' NodeNext
`./data.js` imports resolve from source. This is why the fixtures are not
vendored.
...
All 30 pairs clear 4.5:1 in both schemes, as do the three tag chips against
their own translucent chip backgrounds.
...
the product mark itself, not a redraw.
...
**The routing chip's tether.** On the desktop the routing chip sits inside the
list column, unmistakably about row #1. The phone merges the deck's two panes
into one column of cards, so the chip rides inside the first card as a footer,
under a hairline `Rule` and outside the card's own press target (so "Approve"
cannot also expand the issue).
...
| Route | What it is |
| --- | --- |
| `/` **Ohbox** | New / previously seen (+ a Resurfaced pin group on live sessions), the Screener doorbell, routing rationale, tracker chips. Reading never moves the unread count. |
| `/screener` **Screener** | Three shelves (waiting · screened out · spam). Demo rows carry the fixtures' AI pre-suggestion; a live row honestly has none (no classifier runs client-side). |
| `/sender/[seg]/[id]` | **The decision.** Every held message in full — on live sessions each body hydrates and says so while it is still a snippet — then the bar: five destinations, each a split capsule whose ✓ half files *and* marks read. |
...
| `/message/[id]` | Reading view: why it landed here, what was blocked, the protected-OTP block, the conversation, triage actions. Live opens mark read through the engine and ask for the full text — the pane says when it is still showing the preview. |
...
| `/more` | The desktop rail's lower half: piles, tags (demo), search, settings. |
| `/triage` **Piles** | Answer Later · Parked · Resurface, counts derived from the items. |
| `/tag/[id]` | One tag across every view (demo world). |
| `/search` | Typo-tolerant, on-device, **over the demo corpus** — the screen says so on live sessions. |
| `/settings` | Appearance (real), then the demo world's notifications, mailboxes and tags — labelled as such on live sessions. |
...
- **world / screener / reading** — the demo machine's semantics: "& read" at
  all five destinations, the unread count that must not move, lossless
  held-mail transfer, scroll-seen idempotence, undo as a move-back.
- **no-collapse** — every fixture identity a route puts on screen is
  enumerated, at every depth, and nothing is invented or summarised into an
  "and 3 more".
...
- **privacy** — no banned term anywhere (the list is imported from
  `packages/fixtures`, never restated); network APIs appear only in the
  enumerated connection-seam files, and nothing outside that seam imports the
  engine package.
...
- **live-screens** — the live world layer against a real server: piles match
