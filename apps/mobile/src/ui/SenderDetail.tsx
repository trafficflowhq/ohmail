/**
 * One sender in the Screener: their actual mail, and the decision — mounted by the pushed
 * `/sender/[seg]/[id]` route (one pane) and by the Screener's reading pane (two panes).
 * You never decide about a sender you cannot see: every held message renders in full and
 * the caption states the count and that all of it is shown — no "and 3 more". A live held
 * body starts as its snippet and hydrates; the caption says so under a truncation. "& read"
 * means one thing at all five destinations: the label files the mail, the attached ✓ files
 * it and marks every held message seen first — filing as read moves no unread count.
...
export function SenderDetail({
  ...
  seg: ScreenerSeg;
  /** The STABLE routeKey, never a representative message id — rows are re-minted on new mail. */
  routeKey: string;
  /** Mounted beside the Screener's shelves — no back bar of its own; the pane carries it. */
  inPane?: boolean;
  /** Leaves the detail: back on the route, clearing the selection in a pane. */
  onClose?: () => void;
}) {
  const id = routeKey;
  const t = useTheme();
