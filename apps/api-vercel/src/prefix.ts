 ...
 * `API_PREFIX` from HERE (so the browser rewrite and the stripped prefix cannot drift), which
 * made `@trafficflow/api` part of the web app's program — a package it does not declare —
 ...
 * it; only a clean install scoped to one app — the build server — stops the accident. Keeping
 * this module free of workspace imports keeps it importable. Path normalization only.
 */
