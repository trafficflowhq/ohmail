...
/**
 * The add-time probe SSRF guard — resolve and validate the host, cap the port, before any dial.
 * The probe dials a `host:port` the caller typed: on the hosted service that is a connect oracle
 * into the deployment's own network, so private/loopback/link-local/CGNAT addresses and non-mail
 * ports are refused before the socket opens. On a local install this must not fire — a user's own
 * mail server may sit on a LAN address — so the guard is a policy the host states once: {@link
 * ALLOW_ANY_PROBE_HOST} on the sidecar, {@link makeProbeHostGuard} on Cloud. The resolver is
 * required, with no node:dns default: the test sandbox blocks DNS, so a default would ship the
 ...
 * return is permission. The return value is the pin: a `void` check resolved the hostname,
 * cleared its addresses, and handed the same name to the dialler for a second, independent lookup
 * — a DNS-rebinding server answers the check with a public address and the socket with a private
 * one. A permit is therefore the addresses the dial may connect to, threaded as `ImapConfig.pin`.
 * `null` means "no pin" and belongs to a policy that cleared nothing ({@link
 * ALLOW_ANY_PROBE_HOST}); it is deliberately not an empty array — "I checked nothing" and
 * "nothing is permitted" must not share a value.
 */
...
/**
 * Try the credentials before storing them, and say which thing failed: `POST /mailboxes` used to
 * answer 201 `connected` (the column default) with zero connection attempts. Not a third
 * connector: the other dialers begin from a stored `mailboxId`. Reused below the seam:
 * `ImapAdapter`, the admission counter (a probe is a new login), the failure taxonomy; the
 * classification copies the worker's `classifyMailboxError` order, pinned by a specimen table.
 * Nothing here logs — the input carries a plaintext password and a throw can quote it.
 * `MailboxService.update` is probed; the SMTP block, the sidecar's first-run seal and the
 * worker's env bootstrap surface a bad password on their own paths.
 */
 ...
 * a password typed into the connect form; `accessToken` is the OAuth2 member, minted seconds ago
 * by the Microsoft callback's code exchange, before anything is stored. The OAuth arm carries a
 * token and not a refresh token: a probe is one dial with one login and there is no mailbox id
 * for a token provider to cache against, so the callback simply returns the token it was given.
 * It does not go through `buildImapAuth`: that is the one interpreter of a stored `meta.authType`
 * and its value is failing closed on a row it does not recognise — there is no row here. The
 * password arm still routes through the builder.
 */
...
/**
 * How many probes may be in flight for one address. Two, the same number as
 * `MAX_IMAP_PER_MAILBOX` but not for the same reason: the deadline path releases its slot late
 * (see the `finally` below), and a serverless invocation killed first never releases it. At a cap
 * of one, a single hung probe would lock somebody out of adding their own mailbox until the 90 s
 * reclaim window rolled; the second slot is the retry that gets them through.
 */
export const MAX_PROBES_PER_ADDRESS = 2;

/**
 * How long the whole probe may take before it is abandoned. The connect flow is interactive —
 * somebody is watching this submit button — and a hung probe holds the invocation and shows a
 * spinner that means nothing. Twenty seconds, the outer bound rather than the only one: {@link
 * PROBE_TIMEOUTS} tightens the client's own deadlines below it so the library normally rejects
 * first with a classifiable errno. The race exists because imapflow's socket deadline does not
 * always fire (`imap-types.ts` records one stretching to eight minutes); on expiry the user is
 * told the server accepted the connection and then stopped answering — `err_timeout`'s outcome.
 */
...
/**
 * The classifier, in the same order as the sync worker's `classifyMailboxError`: structural
 * evidence before the flag, because imapflow stamps `authenticationFailed = true` on every
 * failure of the LOGIN command, unconditionally — a flag read first makes every probe below it
 * unreachable and produces a confidently wrong "the mail server refused this password" about a
 * password that is fine. One place this is finer than the worker's: the worker answers `connect`
 * for both "host does not exist" and "host at its connection cap", and here they must diverge —
 * the `connect` class is split by evidence of contact, as two named sets, and an unknown atom
 * falls through to the safe answer (refuse) rather than being read as permission to store.
 */
...
/**
 * imapflow's own constants for "the server hung up on us". Every member implies a socket was
 * established — `ClosedAfterConnectTLS` a completed TLS handshake, `ClosedAfterConnectText` a
 * completed TCP connect, `EConnectionClosed` and `NoConnection` a connection that existed and
 * stopped; `ETHROTTLE` is a tagged reply. A mistyped host cannot produce any of them: it fails at
 * DNS or the SYN ({@link CONNECT_ERRNOS}). This set is what makes the at-cap case work: a
 * provider at its per-account cap sends `* BYE [UNAVAILABLE]` and closes, imapflow discards the
 * bracket atom, and the pending login rejects with a bare connection error carrying no response
 * code to promote.
 */
...
/**
 * Turn a throw from `connect()` into a verdict. Exported for its own test: the order is the part
 * that regresses silently, and a test that can only reach it through a socket is a test written
 * once and never mutated. It is the classifier only, and that is the one way to inherit half of
 * this file: a new call site that wants to try credentials must build {@link makeImapProbe}, not
 * call this — everything that bounds a probe lives in the closure that returns ({@link
 * PROBE_DEADLINE_MS}, the tightened timeouts, the admission acquire/release pair, the
 * close-before-release ordering). Dialling by hand compiles, classifies correctly, and silently
 * has none of them.
 */
...
/**
 * The dial order for one probe. No port → 993 implicit TLS, then 143 STARTTLS — the standard
 * ladder. Port with no mode → that port only, canonical mode first (993 speaks TLS from the first
 * byte; anything else is presumed cleartext-then-STARTTLS), then the other mode on the same port
 * — a user who typed a port is saying where the server is, not how it negotiates, and the
 * wrong-mode rung costs one greeting timeout on the failure path only. Port and mode → exactly
 * that, once: an explicit combination is respected verbatim — it is what every provider preset
 * sends.
 */
...
/**
 * The canonical-host suggestion for a hostname mismatch — the vanity-CNAME shape: a vanity
 * `mail.<own-domain>` host that is a CNAME to the provider's real server, whose chain-valid
 * certificate names the provider's hosts. Not a trust decision: Node validates the chain before
 * the identity, so `ERR_TLS_CERT_ALTNAME_INVALID` — the only path here — means the certificate is
 * trusted and current, wrong only in name. The suggestion is derived from that certificate and
 * only ever shown: the user confirms it, the re-probe dials the confirmed name, and the handshake
 * verifies strictly against it. A spoofed DNS answer can steer the suggestion but never past
 * validation; silently connecting to it would be the trust change this does not make.
 */
...
/**
 * The admission key for an address nothing has been stored for yet. The admission counter keys on
 * a mailbox id and does not hash it — an opaque internal UUID needs no protection. At probe time
 * there is no id and the only stable identity is the account plus the address, which is an
 * identity, so this one hashes (as the table's other identity-bearing namespaces do). Lower-cased
 * before hashing so the same mailbox typed with different capitalisation shares one budget —
 * matching mail 0021's `lower(address)` index; it is a connection budget, not a uniqueness rule.
 * The counter prepends its own namespace: the stored key is `imap:mailbox:probe:<hash>`.
 */
...
/**
 * Race a promise against a deadline. The loser still settles and must not crash the process:
 * `Promise.race` does not cancel the other side, so when the deadline wins, `p` rejects later — a
 * dead socket, an `ETIMEOUT` — with nobody awaiting it, which in Node is an unhandled rejection;
 * an IMAP failure arriving out of band has taken this process down before. The noop catch
 * attached before the race is what makes the abandoned attempt merely abandoned. The timer is
 * cleared on both arms — a probe finishing in 200 ms must not keep a 20 s handle alive — and
 * `unref`'d where the runtime has it.
 */
...
export function makeImapProbe(deps: ApiDeps, opts: ImapProbeOptions = {}): (i: ImapProbeInput) => Promise<ImapProbeVerdict> {
  ...
  return async (input: ImapProbeInput): Promise<ImapProbeVerdict> => {
    ...
    /**
     * One rung: dial, then put the socket down before the next rung may start. The slot may not
     * go back while the socket may still be live: skipping the close on a deadline expiry left
     * the abandoned socket outliving the slot release — the counter said one login while the
     * provider still had two, which is how a probe ladder ends up refused for a mailbox nobody is
     * dialling. Destroying is what makes waiting affordable — `forceClose` is synchronous, so the
     * socket is down by the time the release runs. A dialer with no `forceClose` is a test
    ...
    const dialOnce = async (
      ...
      /**
       * The folder count, inside the same connection and the same deadline: a second dial to
       * count would be a second login against the provider, outside the admission slot's
       * one-login property. A LIST that throws is not a success — it falls through to the same
       * `verdictFor` ladder, so "signed in but your folders could not be read" gets a named
       * refusal rather than a green verdict with no number. A LIST that runs out of budget is
       * reported as a timeout, because that is what it is.
       */
 ...
 * specimen table in the tests is what notices a version moving it.
 */
 ...
 * nodemailer's `_formatError` stamps its own transport code (`ESOCKET`/`ECONNECTION`/`ETLS`) over
 * whatever OpenSSL said, so the code alone reads "socket problem" about a certificate refusal: a
 * hostname mismatch arrives as `code: "ESOCKET"` with Node's full identity sentence intact, and a
 * code-gated version answered `connect` — a true sentence about the wrong thing.
 */
