...
export interface InsertMessageInput {
   ...
   * reads the same row at the same strength in the same transaction and refuses the same stamp,
   * so a read here would be one round trip per message of every first sync. Absent — every caller
   * outside the hosted ingest commit — the door reads the stamp itself. The supplier owes the
   * question whatever this insert does: `sync.ts#commitFenced` sends the standing read when its
   * allocation never happened, which is what keeps the hand-over from being a way past the fence.
   */
