/**
 * `@trafficflow/core/net` — the SSRF gate and its pin, together, and NOT on the package barrel. A
 * subpath because `pinned-fetch.ts` imports `node:http`/`node:https`, and the barrel is imported
 * by graphs that have neither; a consumer opts into node-only code by naming it. The two are
 * exported from ONE entry because they are one mechanism: `assertPublicHttpUrl` returns the
 * addresses it cleared, and `pinnedHttpRequest` is the only correct thing to do with them —
 * split, it would be possible to import the gate, ignore the return value, and hand the hostname
