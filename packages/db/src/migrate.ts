...
/**
 * The migration's advisory lock, in a leaf with no server driver: `concurrent-index.ts` takes the
 * same lock, and the desktop engine reaches it through `search-setup.ts` without this module.
 * Re-exported so every existing import keeps its path.
 */
export { MIGRATION_LOCK_KEY, MIGRATION_LOCK_TIMEOUT_MS } from "./migration-lock.js";
