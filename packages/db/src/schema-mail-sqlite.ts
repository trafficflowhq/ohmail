/**
 * THE MAIL SCHEMA AS SQLITE SPELLS IT — the same tables, the same names, the same constraints.
 *
 * Every identifier this file exports also exists in `schema-mail.ts`, and it is the SAME
 * identifier: services import tables by name and are typed against the Postgres twin, so a build
 * for a device swaps this module in and the service code does not change a line. That is only
 * true while the two stay in step, which is not left to care — `schema-twin-parity.test.ts`
 * compares both twins table by table (names, columns and their order, primary keys, unique and
 * partial indexes, CHECK names) and fails naming the drift.
 *
 * ── WHAT CHANGES, AND WHY EACH IS THE SAME MEANING ────────────────────────────────────────
 *
 * · Identifiers are text. They are generated by the application on both stores, so this is a
 *   change of storage and not of who decides an id.
 * · Instants are integers — milliseconds since the epoch. The query builder hands back a `Date`
 *   on both, so a caller cannot tell them apart, and comparisons order correctly because the
 *   representation is monotonic.
 * · JSON is text read through the JSON functions, with the same defaults.
 * · Booleans are integers with a boolean mode, which is what this store has always had.
 * · Large counters keep their `bigint` type through a column that stores a native 64-bit
 *   integer, rather than the blob the query builder would otherwise choose: a blob does not
 *   compare or order, and these columns are compared and ordered.
 *
 * ── WHAT IS ABSENT, AND WHERE IT WENT ─────────────────────────────────────────────────────
 *
 * The three generated text-search columns have no counterpart here. This store has no stored
 * generated column of that kind, and copying the text into the row to fake one would double what
 * a mailbox costs on a device. Full-text lives in external-content indexes created by the
