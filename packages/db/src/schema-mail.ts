...
export const mailboxes = pgTable("mailboxes", {
  ...
  /**
   * WHEN THIS INSTALL LAST SIGNED OUT OF THIS MAILBOX — the sign-out fence's durable half.
   *
   * Written in the same transaction as the credential delete; every writer that seals a password
   * records this value before it dials and re-reads it under this row's lock, so a value that
   * MOVED means a sign-out landed while the password was being checked. An in-memory epoch cannot
   * say that — it belongs to one process and a credential is on disk. NULL is "nobody has signed
...
export const mailboxFolders = pgTable("mailbox_folders", {
   ...
   * the pair; NULL is "not where the last pass stopped", which is every other row. The adapter
...
export const idempotencyKeys = pgTable("idempotency_keys", {
   ...
   * recipients — so an erasure that does not reach it leaves the mailbox readable through a
   * retry. The erasure replaces the body with a marker and stamps this; the replay then 410s
   * rather than describing something that no longer exists. A STAMP and not a delete: a delete
...
export const awayResponders = pgTable("away_responders", {
  ...
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
  /**
   * When the away pass last WALKED this responder — the rotation. The probe orders by it first,
   * so the accounts a run entered sort last and the ones behind them come up next; without it
   * the page is the same fifty every tick and the fifty-first never answers anybody. The epoch
   * means NEVER WALKED, which every row starts at. NOT `updated_at`: that is the away episode
   * key (`away_responder_sent.responder_updated_at`), and moving it once a minute would open a
   * new episode each tick and re-answer every correspondent. The default is the INSTANT, not
   * the migration's own `to_timestamp(0)` — a Postgres-only function in a module the engine
   * conveys to a store that has no such function.
   */
...
export const accountSettings = pgTable("account_settings", {
   ...
   * LAST in the declaration: `ALTER TABLE … ADD COLUMN` appends, and the twins are compared
