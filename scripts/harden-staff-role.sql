...
-- `notified_signature` (cloud 0025) is the renotify policy's condition signature and
-- `claimed_until` (cloud 0026) is the notify claim's lease — the ninth and tenth columns,
-- each added by the explicit three-place decision this file demands: the pass's claim names
-- both and the settle writes both, so the API driver's pass raises 42501 on every run without
-- all three verbs on each. The signature is composed by `alerts.ts` from an alert's severity
...
GRANT SELECT (alert_key, kind, severity, opened_at, last_seen_at, notified_at, notify_count, detail, notified_signature, claimed_until, cls, affected_accounts, fix_href, title, "count", resolved_at)
  ON public.alert_state TO ohmail_admin;
GRANT INSERT (alert_key, kind, severity, opened_at, last_seen_at, notified_at, notify_count, detail, notified_signature, claimed_until, cls, affected_accounts, fix_href, title, "count", resolved_at)
  ON public.alert_state TO ohmail_admin;
